...
public class Controller extends HttpServlet {
    ...
    public void init(ServletConfig config) throws ServletException {
    	...
    	try {
    		//config.getServletContext().getRealPath("/") : 웹애플리케이션폴더
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			//Class getClass() : Object의 멤버 메서드. 클래스 정보를 리턴
			//action.getClass() : Action 클래스의 정보
			// getMethod(메서드이름, 파라미터타입) : 클래스 정보에서 method 정보
		Method method = action.getClass().getMethod(methodName, paramType);
		//invoke(호출대상 객체, 매개변수값(인자)) : 메서드 호출 
		//forward : new ActionForward(flase, "ex15_model2/hello.jsp")
