...
public class Main2_Student {
	...
	public static void main(String[] args) {
		//Connection 객체를 이용하여 mybatis와 db를 연결
		SqlSession session = sqlMap.openSession();
		System.out.println("학생 테이블에 레코드 추가하기");
		Student st = new Student(); //db의 student 테이블의 컬럼과 같은 프로퍼티를 가지는 클래스
		st.setStudno(1002);
		st.setName("김삿갓");
		st.setGrade(1);
		st.setId("kimsg2");
		...
		//파라미터 값을 이용하여 db에 새로운 레코드 추가. 추가된 레코드건수 리턴
		...
		System.out.println("update 레코드 건수 :" + cnt);
		System.out.println(stResult);
		//1002번 학생 정보 삭제하기
		//student.delete
