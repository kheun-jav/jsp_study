...
public class Main1 {
	public static void main(String[] args) {
		...
		//selectOne(sql구문의 id값) : 결과레코드가 한건인 경우.
		...
		//selectList(sql의id값) : 결과레코드가 여러건인 경우
		...
		System.out.println("member 테이블에서 이름에 1을 가진 레코드 즈회하기=====");
		list = session.selectList("member.selectname1","%1%");
		for(Member m : list) System.out.println(m);
		System.out.println("member 테이블에서 이름에 1을 가진 레코드 즈회하기=====");
		list = session.selectList("member.selectname2","1");
		for(Member m : list) System.out.println(m);
		System.out.println("member 테이블에서 이름에 1을 가진 레코드 중 남자정보 즈회하기=====");
