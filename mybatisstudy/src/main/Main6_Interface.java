...
public class Main6_Interface {
	...
	public static void main(String[] args) {
		...
		for(Student s : list) System.out.println(s);
		System.out.println("키가 175 이상인 학생 정보 조회하기");
		map.clear();
		map.put("height", 175);
		list = session.getMapper(cls).select(map);
		for(Student s : list) System.out.println(s);
		System.out.println("몸무게가 60 이하인 학생 정보 조회하기");
		map.clear();
		map.put("weight", 60);
		list = session.getMapper(cls).select(map);
		for(Student s : list) System.out.println(s);
		...
		list = session.getMapper(cls).select2(map);
		for(Student s : list) System.out.println(s);
		System.out.println("1학년 학생 중 키가 175 이상인 학생 정보 조회하기");
		map.clear();
		map.put("grade", 1);
		map.put("height", 175);
		list = session.getMapper(cls).select2(map);
