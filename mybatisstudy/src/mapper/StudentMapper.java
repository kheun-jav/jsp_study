 ...
 * 3. 메서드의 이름의 sql문장의 key값임 => 같은 이름의 메서드 허용안됨
 * 	  => Mapper로 사용하는 인터페이스는 오버로딩 안됨
 */
public interface StudentMapper {

	@Select("select * from student")
	List<Student> select();
	// Mapper 메서드는 오버로딩 절대 안됨!
//	@Select("select * from student")
//	List<Student> select(int studno);

	@Select("select * from student where grade=#{value}")
	List<Student> grade(int i);
	...
	//@Param("grade")int a : a 변수를 grade 라는 key값으로 설정 
	@Select("select * from student where grade=#{grade} and height >= #{height}")
