...
<%-- /webapp/exam1.jsp 
	수출입은행 환율 정보 중 EUR, JPY(100), CNH, USD 통화만 
	ex02_exchange.jsp 작성한 형태로 출력하기
--%>
<!DOCTYPE html>
<html>
<head>
...
<%
	...
	List<String> title = Arrays.asList("전신환받으실때", "전신환보내실때",
	...
	try {
		...
		for(Element tr : trs) {
			...
			// 필터링 치고싶으면 tdlist에서 trlist로 옮기기 전에 조건문 사용
			if(tdlist.size() >0) {
	...
	<c:forEach items="${trlist}" var="tdlist">
		<c:forEach items="${tdlist}" var="td" varStatus="stat">
			<c:choose>
				...
				<c:when test="${stat.index % 8 == 1 }"> <%-- 두번째 td : 국가/통화명 --%>
					<td rowspan="6">${td}</td>
				</c:when>
				<c:when test="${stat.index % 8 == 2 }"> <%-- 세번째 td : 전신환받으실때 환율 --%>
